...
exports.getExcelData = async (filters) => {
  return knex('issueticket as it')
    ...
    .leftJoin(
      knex.raw('tickettag tg ON tg.tickettag_id = ANY(it.ticket_tag)')
    )
    ...
    // ---------- FILTERS ----------
    .modify((qb) => {
      if (filters.project_id && filters.project_id !== 'undefined') {
        console.log("Project ID filter:", filters.project_id);
        qb.where('it.project_id', Number(filters.project_id));
      }
